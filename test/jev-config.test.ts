...
test("Jev scope comes only from owner-global configuration, never project overrides", () => {
	...
	const owner = { enabled: false, project, files: ["source.ts"], dropBelow: null, auto: false, task: "" };
...
test("auto scoring decodes only with a standing task brief", () => {
	const config = lenientConfig();
	config.jev = { enabled: false, project: "", files: [], dropBelow: null, auto: true, task: "" };
	assert.equal(new JevSelection(config).options, null);
	config.jev = { ...config.jev, task: "Standing brief" };
	assert.deepEqual(new JevSelection(config).options, config.jev);
});
